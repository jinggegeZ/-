            ...
            <div class="hotcitybox">
                ...
                <div class="hotcitybox-body">
                  <div class="hotcitybox-body-item" v-for="(item,index) in more" :key="index">
                    <div class="body-item-img1"><img class="body-item-img" :src="item.photos[0].url" alt=""></div>
                    <div class="body-item-descriton">
                      <div class="body-item-descritonfont">{{item.name}}</div>
                      <div>pingfen</div>
                      <div>{{item.adname}}<span>{{item.address}}</span></div>
                      <div>renjun24</div>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  mounted() {
    ...
    //获取商铺详情
    this.$api.product({city:this.city,keyword:this.name}).then(res => {
      if(res.code === 200){
