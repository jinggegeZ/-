            ...
            <div class="detail-topbox-leftbox">
              <el-select v-model="value" placeholder="全部分类">
                <el-option
                  ...
                  :value="item.name"
                >
                  <span style="float: left" v-if="Array.isArray(item)"><span v-for="(item1,index1) in item.name" :key="index1">{{item1}}</span></span>
                  <span style="float: left" v-else-if="">{{ item.name }}</span>
