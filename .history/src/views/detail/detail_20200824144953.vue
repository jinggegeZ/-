                ...
                >
                  ...
                  <span style="float: left" v-if="Array.isArray(item.name)">
                    <span v-for="(item1,index1) in item.name" :key="index1" >
                        <span ref="changes" :class="{'changeColor':flagColor===index}" @mouseenter="changeItem(index1)" @mouseleave="deleteitem(index1)">{{item1}}</span><span v-if="index1 !==  item.name.length-1">/</span>
                    </span>
                  </span>
                  <span class="hover" style="float: left" v-else>{{ item.name }}</span>
...
export default {
  ...
  methods: {
      changeItem(index1){
               item.name.map((item2,index2)=>{
                    if(index2 === index1){
this.refs.changes.style.color = orange
                    }
               })
